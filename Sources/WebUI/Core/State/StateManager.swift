...
public final class StateManager: @unchecked Sendable {
    ...
    public func getState<T: Codable & Sendable>(key: String, scope: StateScope, as type: T.Type) -> T? {
        return scopedStorage.getState(key: key, scope: scope, as: type)
    }
    ...
    public func subscribe<T: Codable & Sendable>(to key: String, scope: StateScope, callback: @escaping (T) -> Void) -> StateSubscriptionToken {
        return scopedStorage.subscribe(to: key, scope: scope, callback: callback)
    }
    ...
    public func getDebugHistory() -> [DebugHistoryEntry] {
        return scopedStorage.getDebugHistory()
    }
    ...
    public func exportStateJSON() -> String {
        return scopedStorage.exportStateJSON()
    }
...
private final class ScopedStateStorage: @unchecked Sendable {
    ...
    func getState<T: Codable & Sendable>(key: String, scope: StateScope, as type: T.Type) -> T? {
        lock.withLock {
            return statesByScope[scope]?[key] as? T
        }
    }
    ...
    func subscribe<T: Codable & Sendable>(to key: String, scope: StateScope, callback: @escaping (T) -> Void) -> StateSubscriptionToken {
        lock.withLock {
    ...
    func importStateJSON(_ json: String) {
        lock.withLock {
            guard let data = json.data(using: .utf8),
                  let importData = try? JSONSerialization.jsonObject(with: data) as? [String: [String: Any]] else {
                return
            }
