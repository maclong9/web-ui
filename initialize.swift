...
enum InitError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .invalidTemplate(let value):
            return "Invalid template '\(value)'. Valid options: \(Template.allCases.map { $0.rawValue }.joined(separator: ", "))"
...
func downloadAndExtractTemplate(to directory: URL) throws {
    let templateURL = "https://github.com/maclong9/web-ui/archive/refs/heads/main.tar.gz"
...
func replaceContentIn(file path: String, projectName: String) {
...
do {
    ...
    // Setup paths
    let templatePath = tmpDir.appendingPathComponent("web-ui-main/examples/\(template.rawValue)")
    ...
    do {
        try FileManager.default.copyItem(at: templatePath, to: URL(fileURLWithPath: destination))
    } catch {
        throw InitError.fileCopyFailed(destination)
    }
    
    // Replace placeholders in files
    replaceContentIn(file: "\(destination)/Package.swift", projectName: projectName)
    replaceContentIn(file: "\(destination)/Sources/Application.swift", projectName: projectName)
    ...
    if template == .static {
        print("Run \u{001B}[1;33mcd \(projectName) && swift run\u{001B}[0m to build your static site to the \u{001B}[1;36m.output\u{001B}[0m directory.\n")
    } else {
        print("Run \u{001B}[1;33mcd \(projectName) && swift run\u{001B}[0m to start your server.\n")
    }
