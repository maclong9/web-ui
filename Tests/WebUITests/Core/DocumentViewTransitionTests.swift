...
struct DocumentViewTransitionTests {
    ...
    func testDocumentGeneratesCSSForAllTransitionTypes() async throws {
        ...
        for transitionType in transitionTypes {
            ...
            #expect(
                css!.contains("@view-transition"),
                "CSS should contain @view-transition for \(transitionType)")
            #expect(
                css!.contains("animation-duration: 300ms"),
                "CSS should contain duration for \(transitionType)")

            if transitionType != .none {
                #expect(
                    css!.contains("@keyframes"),
                    "CSS should contain keyframes for \(transitionType)")
            }
        }
    }
